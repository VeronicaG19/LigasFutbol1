...
class RequestNewTeamCubit extends Cubit<RequestNewTeamState> {
  RequestNewTeamCubit(this._userRequestsService,
      this._leagueService, this._categoryService,)
  ...
  Future<void> availableLeagues() async {
    ...
    response.fold(
      ...
      emit(state.copyWith(screenStatus: ScreenStatus.loaded, leagueList: r, leagueSelect: r[0]));
  ...
  Future<void> availableCategoriesByLeagueId(int leagueId) async {
    ...
    response.fold(
            ...
            screenStatus: ScreenStatus.error,
            errorMessage: l.errorMessage)), (r) {
      print("Categorias--> ${r.length}");
      emit(
          state.copyWith(screenStatus: ScreenStatus.loaded, categoryList: r, categorySelect: r[0]));
    });
  }
  ...
  Future<void> sendRequestNewTeam({required int? partyId}) async{
    ...
    final response = await _userRequestsService.sendRequestNewTeam(requestToLeagueDTO);
    response.fold(
            (l) => emit(state.copyWith(
