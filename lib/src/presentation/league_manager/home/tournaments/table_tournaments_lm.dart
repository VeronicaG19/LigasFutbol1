...
class TableTournamentLm extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
        ...
        child: BlocBuilder<TournamentLmCubit, TournamentLmState>(
            ...
            return  ListView.builder(
              ...
              itemBuilder: (context, index){
                return  Column(
                    ...
                    categoria: "${state.tournamentList[index].categoryId?.categoryName}",
                    //modalidad: "${state.tournamentList[index].typeOfGame}",
                    nEquipos: state.tournamentList[index].maxTeams ?? 0,
                    fecha: "${state.tournamentList[index].inscriptionDate?? '-'}",
                    estado: "${state.tournamentList[index].statusBegin}",
                    visibilidad: "${state.tournamentList[index].tournamentPrivacy}"
                    ),
                    ]
                );
              }
